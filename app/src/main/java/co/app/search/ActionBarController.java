...
/*
    ...
    private final AnimUtils.AnimationCallback fadeOutCallback =
    ...
    *//** Called when search UI has been exited for some reason. *//*
    public void onSearchUiExited() {
        ...
        if (searchBox.isExpanded()) searchBox.collapse(true *//* animate *//*);
        if (searchBox.isFadedOut()) searchBox.fadeIn();
    ...
    *//**
    ...
    public void onDialpadDown() {
        LogUtil.d(
                ...
                searchBox.isExpanded());
        if (activityUi.isInSearchUi()) {
            if (searchBox.isFadedOut()) searchBox.setVisible(true);
    ...
    *//**
    ...
    public void onDialpadUp() {
        ...
        if (activityUi.isInSearchUi()) slideActionBar(true *//* slideUp *//*, true *//* animate *//*);
        else searchBox.fadeOut(fadeOutCallback);
    }
    ...
    public void slideActionBar(boolean slideUp, boolean animate) {
        ...
        if (animate) {
            ...
            animator.start();
        } else setHideOffset(slideUp ? activityUi.getActionBarHeight() : 0);
    ...
    *//** Restores the action bar state from a provided {@link Bundle}. *//*
    public void restoreInstanceState(Bundle inState) {
        ...
        if (isSearchBoxFadedOut) {
            if (!searchBox.isFadedOut()) searchBox.setVisible(false);
        } else if (searchBox.isFadedOut()) searchBox.setVisible(true);

        final boolean isSearchBoxExpanded = inState.getBoolean(KEY_IS_EXPANDED);
        if (isSearchBoxExpanded) {
            if (!searchBox.isExpanded()) searchBox.expand(false, false);
        } else if (searchBox.isExpanded()) searchBox.collapse(false);
    }
